  ...
  <div class="homeWap clearfix" id="huacengPar">
    <div class="pro-details-left">
              	...
              	<span>
                    <c:choose>
                        <c:when test="${loanInfo.productStatus eq 0}">
                            剩余募集金额 ${loanInfo.leftProductMoney}元
                        </c:when>
                        <c:otherwise>
                            已满标
                        </c:otherwise>
      ...
      <div class="pro-details-cnt">
        ...
        <c:otherwise>
            <c:forEach items="${bidInfoList}" var="bil" varStatus="sta" >
                <dd>
                    <span class="record-num">${sta.count}</span>
                    <span class="invest-user">${fn:substring(bil.user.phone, 0, 3)}*****${fn:substring(bil.user.phone,8 ,11)}</span>
                    <span class="invest-money">${bil.bidMoney}</span>
                    <span class="invest-time"><fmt:formatDate value="${bil.bidTime}" pattern="yyyy-MM-dd HH:mm:ss" /></span>
                </dd>
            </c:forEach>
          ...
          <dl class="usable">
              ...
              <dd>资金(元)：
                  <c:choose>
                      ...
                      <span style="font-size:18px;color:#ff6161;vertical-align:bottom;"><a href="login.jsp">请登录</a></span>
                    </c:when>
                      <c:otherwise>
